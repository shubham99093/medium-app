...
const AppBar = () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${BACKEND_URL}/api/v1/blog/`,
        {
          ...
          title: title,
          content: content, // Replaced description with content
        },
        {
          headers: {
      ...
      navigate(`/post/${updateData?.postId}`);
  ...
  return (
    <header className="border-b border-gray-200 z-100">
      <div className="size h-[60px] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <Search blur={SearchBlur} setBlur={setSearchBlur} />
        </div>
        <div className="flex items-center gap-3 sm:gap-7">
          <span
            onClick={() => setSearchBlur(true)}
            className="flex sm:hidden text-3xl  text-gray-500 cursor-pointer"
          >
            <CiSearch />
          </span>
          {pathname === "/new-story" || editPath ? (
            <button
              onClick={editPath ? handleEdit : () => setPublish(true)}
              className="btn !bg-green-700 !py-1 !text-white !rounded-full"
            >
              {editPath ? "Update" : "Publish"}
            </button>
          ) : (
            <Link
              to={"/new-story"}
